...
The top four architectures sit within $0.2$~mm of one another, a spread
well inside the training-variance floor discussed in
Section~\ref{sec:discussion}, and cluster around the published CAMUS
...
\textbf{nnU-Net} produces the most accurate EF estimates (MAE $= 5.52\%$,
$r = 0.828$, Bland--Altman bias $+0.84\%$, 95\% limits of agreement
$[-14.2, +15.9]\%$, the narrowest of any architecture tested). The near-zero mean bias is comparable to reported
...
parity. UNet-V1 and UNet-ResNet
(both MAE $= 6.78\%$) follow, with UNet-ResNet attaining the
highest EF correlation ($r = 0.832$, marginally ahead of nnU-Net's
$0.828$). Notably, the best-Dice architecture is not the best on the
clinical axis: TransUNet reaches only MAE $= 7.64\%$ ($r = 0.799$),
behind three CNNs that score lower on bulk overlap. The Transformer-only Swin-UNet shows
