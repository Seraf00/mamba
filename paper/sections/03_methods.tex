...
subject of a companion study.

\input{tables/T0_archs}
...
\item Training hardware: Google Colab Pro NVIDIA L4 (24~GB) and A100
      (40~GB) instances. The final test-set evaluation reported in
      Section~ef{sec:results} was run in a single pass on one NVIDIA
      RTX 4060 Laptop GPU, so that every metric in every table comes from
      the same inference run.
\end{itemize}

All nine architectures were trained with a \emph{uniform} batch size of
$32$ --- the largest value that fits every model within the training-GPU
memory budget --- rather than a per-architecture batch size, so that the
optimisation regime is held as constant as possible across the comparison.
We did \emph{not} perform per-architecture hyperparameter search: the goal
is explicit cross-architecture comparison under one shared protocol.
...
Each CAMUS patient carries an expert-assigned image-quality grade
(Good / Medium / Poor). A quality-stratified robustness breakdown is
deferred to the extended version of this work.
