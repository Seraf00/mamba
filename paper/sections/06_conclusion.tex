...
($4.08$~mm). The pure-CNN nnU-Net leads on EF accuracy
(MAE $5.52\%$, $r = 0.828$) and is $6.5\times$ more parameter-efficient.
...
across the top four architectures) reproduces the originally reported
CAMUS nnU-Net baseline of $4.3$~mm rather than improving on it, while EF
error falls from the $\sim$$9\%$ originally reported to $5.52\%$. We read
this as a useful calibration point for cross-paper comparison: on CAMUS,
a decade of protocol modernisation is visible in the downstream clinical
quantity but not in the contour itself, which appears to be approaching
the ceiling set by annotation variability.
