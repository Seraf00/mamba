...
($0.8882$) but visibly worse on EF (MAE $10.46\%$, $r = 0.574$). This
...
unified protocol reaches HD95 $4.27$~mm and EF MAE $5.52\%$. The two axes
therefore behave very differently: boundary accuracy is statistically
indistinguishable from the 2019 result, while EF error falls by roughly
$40\%$.

We read this as evidence that the contour itself is close to the ceiling
imposed by annotation quality on CAMUS --- inter-observer variability on
this dataset is of the same order as the differences we measure between
architectures --- whereas the downstream clinical quantity still has room
to improve. Where EF does improve, it is \emph{not} an architectural
effect: nnU-Net's architecture is unchanged. It is attributable to three
...
\item \textbf{Pretrained encoders.} Five of the nine architectures
      ...
      pretrained Swin-UNet, FPN-UNet, and DeepLabV3+ --- and nnU-Net also
      attains the best EF agreement at roughly one-sixth of TransUNet's
      parameter count. Encoder pretraining thus appears to help mainly where
